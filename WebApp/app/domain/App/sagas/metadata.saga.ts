...
import { requestApprovalApi, uploadMetadataApi, updateDappApi, retrieveMetadataApi, retrieveAllDappsMetadataApi } from 'api/api';
...
export const retrieveAllDappsMetadata = async () => {
	...
	for (let i = 0; i < metadataHashes.length; i++) {
		const convertedDappMetadataHash = getBytes32FromIpfsHash(
			metadataHashes[i],
		)
