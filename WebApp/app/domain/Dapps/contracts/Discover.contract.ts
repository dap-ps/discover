...
// Transaction methods
export const DiscoverCreateDApp = async (dappId: string, tokenAmount: BigNumber, uploadedMetadata: any) => {
...
export const DiscoverSetMetadata = async (id: string, metadata: any, email: string) => {
  const account: string = await getAccount()
  if (account == AddressZero) {
    throw 'Account not connected'
  }
...
export const validateUpVoteEffect = async (id: string, amount: number) => {
  ...
  if (
    Number(dapp.balance) + bigNumberify(amount).div(defaultMultiplier).toNumber() >
    safeMax
  ) {
    throw new Error(
