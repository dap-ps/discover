...
import { Theme, createStyles, withStyles, WithStyles, Typography, FormHelperText } from '@material-ui/core';
import { FieldProps, getIn } from 'formik';
import { appColors, uiConstants } from 'theme';
import { DAPP_CATEGORY_ICONS, DAPP_CATEGORY, DAPP_CATEGORY_STRINGS } from 'utils/constants';
  ...
  return <section className={classes.root}>
    ...
    <div className={classes.internal} ref={ref}>
      <div className={classNames(classes.options, open ? "active" : "")}>
        {
          Object.keys(DAPP_CATEGORY_ICONS)
          .map((key: DAPP_CATEGORY, index: number) =>
            <div className={classes.item} onClick={() => setCategory(key)} style={{backgroundColor: appColors.sections[key].base}} key={`cat-selector-${index}`}>
      ...
      <div className={classNames(classes.item, classes.selected, appColors.sections[field.value] ? field.value : "unset")} onClick={() => setOpen(true)}>
        {appColors.sections[field.value] ? DAPP_CATEGORY_ICONS[field.value].minimal({}) : false}
        <Typography>
          {field.value ? DAPP_CATEGORY_STRINGS[field.value] : "Choose category"}
        </Typography>
        <DropDownArrowsIcon />
      </div>
    </div>
